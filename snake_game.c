...
void move_menu() {
    do{  
        ...
        if (kbhit()) command = getch();             //pegando o teclado sem parar o programa
...
void running() {
    ...
    do{
        if (option == 7) new_game();                //se for selecionado novo jogo
        else if (option == 9) continue_game();      //se for selecionado continuar
        else if (option == 11) difficulty_menu();   //se for selecionado dificuldade
        else if (option == 13) highscore_menu();    //se for selecionado records
        else if (option == 15) user_menu();         //se for selecionado usuário
        else if (option == 17) return 0;            //se for selecionado para sair
    } while (command != 13);
}
...
void difficulty_menu() {
    ...
    switch (option) {
        case 7:
            ...
            //atulizando o modo com a dificuldade correspondente
            for (int count = 0; count < 7; count++) mode[count] = mode1[count];    
            break;
        
        case 9:
            speed = 100;  //dificuldade padrão com tempo de resposta normal
            //atulizando o modo com a dificuldade correspondente
            for (int count = 0; count < 7; count++) mode[count] = mode2[count];    
            break;
        
        case 11:
            speed = 25;  //caso seja selecionado dificil o game tem o tempo de resposta e exewcução mais rápido
            for (int count = 0; count < 7; count++) mode[count] = mode3[count];
            break;
    }
    erase_menu();
    menu();
...
int move_player() {
    for (int count = 0; count < 20; count++) {
        ...
        if (save == 1) game_over();                 //caso a colisão no próprio jogador aconteça
...
void collision() {
    for (int count = 0; count < 19; count++) { //salvando as corrdenadas do jogador para verificar se são iguais
        ...
        if (colune_player == player[19][0])  //caso seja igual a parte incial do jogador, a colisão aconteceu
            if(line_player == player[19][1]) save = 1;
    }
}
    ...
    switch(y) {
        ...
        case 2:                                         //caso surge um segundo melhor recorde
            ...
            for (int count = 0; count < 20; count++) {        //atribuindo o novo segundo melhor recorde
                ...
                if (count < 7) h2_mode[count] = mode[count];
            }
            highscore2 = score;
            break;
        ...
        case 3:
            for (int count = 0; count < 20; count++) {        //atribuindo o novo segundo melhor recorde
                ...
                if (count < 7) h3_mode[count] = mode[count];
            }
            highscore3 = score;
            break;
    }
